...
export default async function handler(
  ...
  try {
    const { dog_id, lat, lng } = req.body;

    if (!dog_id || lat === undefined || lng === undefined) {
      return res.status(400).json({ error: "Missing required fields" });
    }

    // Get boundaries for this dog
    ...
    if (!boundaries || boundaries.length === 0) {
      return res
        .status(200)
        .json({ inside: true, message: "No boundaries set" });
    }
    ...
    res.status(200).json({
      inside: isInsideAnyBoundary,
      message: isInsideAnyBoundary ? "Inside boundary" : "Outside boundary",
    });
  } catch (error: any) {
